...
local function launchOrFocus(app)
	if app.bundleIDs then
		for _, bid in ipairs(app.bundleIDs) do
			if hs.application.launchOrFocusByBundleID(bid) then
				return true
			end
		end
	end
	...
	local function applyRatio(ratioID)
		return function()
			if not state.setKey then
				showModalAlert(
					"Tile: まずアプリを2つ(または3つ)選択してください (c/w/s/o/f)",
					statusAlertSeconds
				)
				return
			end
...
local CHROME_PROFILES = {
	...
	{ key = "y", dir = "Profile 4", name = "profile_yellow", menu = "profile_yellow" },
...
local function switchToChromeProfile(p)
	...
	if not chrome then
		...
		hs.task
			.new("/Applications/Google Chrome.app/Contents/MacOS/Google Chrome", nil, { "--profile-directory=" .. p.dir })
			:start()
		return
	end
...
do
	...
	local function nameMatches(app)
		local n = app:name()
		if not n or not QUIT_ALL.excludedNames then
			return false
		end
	...
	local function isExcluded(bundleID)
		...
		if QUIT_ALL.excludedBundleIDPrefixes then
			for _, pfx in ipairs(QUIT_ALL.excludedBundleIDPrefixes) do
				if hasPrefix(bundleID, pfx) then
					return true
				end
			end
		end
		if QUIT_ALL.excludeFinder and bundleID == "com.apple.finder" then
			return true
		end
		return false
	end
