...
-- Sticky note overlay for terminal task labels.
do
  local noteDir = os.getenv("HOME") .. "/.cache/wezterm-note"
  local noteFile = noteDir .. "/current.txt"
  local overlay = { bg = nil, text = nil }
  local noteStyle = {
    font = { name = "Helvetica Neue", size = 16 },
    color = { white = 0.08, alpha = 1.0 },
    paragraphStyle = { lineBreak = "wordWrap", alignment = "left" },
  }

  os.execute('/bin/mkdir -p "' .. noteDir .. '"')
  ...
  local function readNote()
    local f = io.open(noteFile, "r")
    if not f then return "" end
    local text = trim(f:read("*a") or "")
    f:close()
    return text
  end

  local function targetScreenForNote()
    local win = hs.window.frontmostWindow()
    if win and win:screen() then return win:screen() end
    return hs.screen.primaryScreen()
  end
  ...
  local function ensureOverlay()
    if overlay.bg and overlay.text then return end

    overlay.bg = hs.drawing.rectangle(hs.geometry.rect(0, 0, 10, 10))
    overlay.bg:setRoundedRectRadii(18, 18)
    overlay.bg:setFillColor({ red = 1.0, green = 0.93, blue = 0.56, alpha = 0.96 })
    overlay.bg:setStroke(false)
    overlay.bg:setLevel(hs.drawing.windowLevels.status)
    overlay.bg:setBehaviorByLabels({ "canJoinAllSpaces", "stationary" })

    overlay.text = hs.drawing.text(hs.geometry.rect(0, 0, 10, 10), "")
    overlay.text:setTextStyle(noteStyle)
    overlay.text:setLevel(hs.drawing.windowLevels.status)
    overlay.text:setBehaviorByLabels({ "canJoinAllSpaces", "stationary" })
  end

  local function hideOverlay()
    if overlay.bg then overlay.bg:hide() end
    if overlay.text then overlay.text:hide() end
  end

  local function showNoteOverlay()
    local note = readNote()
    if note == "" then
      hideOverlay()
      return
    ...
    local screen = targetScreenForNote()
    local frame = screen:frame()
    local maxWidth = math.min(420, math.floor(frame.w * 0.3))
    local textSize = hs.drawing.getTextDrawingSize(note, noteStyle)
    local width = math.max(220, math.min(maxWidth, textSize.w + 30))
    local contentHeight = math.max(36, math.ceil(textSize.h / math.max(1, textSize.w) * width))
    local height = math.max(64, math.min(220, contentHeight + 26))
    local x = frame.x + frame.w - width - 18
    local y = frame.y + 18

    overlay.bg:setFrame(hs.geometry.rect(x, y, width, height))
    overlay.text:setFrame(hs.geometry.rect(x + 14, y + 10, width - 28, height - 20))
    overlay.text:setText(note)
    overlay.bg:show()
    overlay.text:show()
  end

  hs.pathwatcher.new(noteDir, function()
    showNoteOverlay()
  end):start()
  hs.screen.watcher.new(showNoteOverlay):start()
